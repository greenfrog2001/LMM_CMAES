...
class CMA:
    ...
    def __init__(
        ...
        self._eff = mu_eff
        self._mu_eff_minus  = mu_eff_minus
    ...
    def update_weights(self):
        ...
        weights_prime = np.array(
            [
                math.log((self._mu + 1) / 2) - math.log(i + 1)
        ...
        min_alpha = min(
            ...
            1 + (2 * self._mu_eff_minus) / (self._mu_eff + 2),  # eq.51
            (1 - self._c1 - self._cmu) / (self._n_dim * self._cmu),  # eq.52
        )
        ...
        self._weights  = np.concatenate([self._weights, np.zeros(self._popsize - self._mu)])
    ...
    def tell(self, solutions: List[Tuple[np.ndarray, float]], lmm = False) -> None:
        """Tell evaluation values"""
        if not lmm:
